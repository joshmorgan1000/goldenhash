...
uint64_t find_modulo_prime(uint64_t n, uint64_t modulo) {
    uint64_t target = (n * PHI) / modulo;
    if (is_prime(target)) {
        return target;
    }
    for (uint64_t delta = 1; delta < target; ++delta) {
        if (is_prime(target - delta)) {
            return target - delta;
        }
        if (is_prime(target + delta)) {
            return target + delta;
...
private:
    ...
    uint64_t main_factor;    // Main factor for mixing
    std::vector<uint64_t> factors;
    std::vector<uint64_t> secret;  // Modular secret for mixing
    std::unique_ptr<GoldenHashN> golden_hash_n;  // Golden hash N instance
...
public:
    ModularGoldenHash(uint64_t table_size) : N(table_size) {
        ...
        if (is_prime(N)) {
            ...
            working_mod = N + 1;
            // Should not happen... because we are not going to input prime numbers
        } else {
        ...
        main_factor = table_size / working_mod;
        size_t secret_size = main_factor * 3;
        secret.resize(secret_size);

        uint64_t prime_distance = find_modulo_prime(N, working_mod);
        
        // Generate secret values using golden ratio mixing
        uint64_t h = N;
        for (size_t i = 0; i < secret_size; i++) {
            h = h * prime_high + i;
            h = (h + main_factor) % working_mod;
            h = (h * prime_low) % working_mod;
            h = (h + prime_distance) % working_mod;
            // Already in modular space
            secret[i] = h;
        }
    }

    std::string to_binary(uint8_t* data, size_t size) const {
        std::stringstream ss;
    ...
    uint64_t update(uint64_t h, const std::string& data) const {
        // Process each byte with secret mixing
        for (size_t i = 0; i < data.size(); i++) {
            // Mix with secret value (cycles through 24 values)
            uint64_t secret_val = secret[i % secret.size()];
            
            // Input mixing with secret
    ...
    uint64_t finalize(uint64_t h) const {
        uint64_t mod_prime = find_modulo_prime(N, working_mod);
        // Final mixing with prime
        return (h * mod_prime) % working_mod;
    }
    
    uint64_t hash(const std::string& data) const {
        uint64_t h = 0;
        for (char c : data) {
            h = h * prime_high + static_cast<uint8_t>(c);
        }
        // Update with data
        h = update(h, data);
        return finalize(h);
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < test_data.size(); i++) {
        ...
        uint64_t h = hasher.hash(std::string(data.begin(), data.end()));
        hash_counts[h]++;
        ...
        if (i % 100 == 0) {
            ...
            for (size_t byte_idx = 0; byte_idx < data.size() && byte_idx < 32; byte_idx++) {
                for (int bit = 0; bit < 8; bit++) {
                    ...
                    // Hash modified
                    uint64_t h2 = hasher.hash(std::string(modified.begin(), modified.end()));
    ...
    for (const auto& vec : vectors) {
        uint64_t h = hasher.hash(vec.str);
